...
class Decoder(nn.Module):
    def __init__(self, image_size: Tuple[int, int, int] = (1, 64, 64),
        ...
        if image_size == (3, 128, 128):
            ...
            self.cnn_layers = nn.Sequential(
                nn.ConvTranspose2d(self.in_channels, self.hidden_channels, **cnn_kwargs), nn.ReLU(),
                nn.BatchNorm2d(self.hidden_channels),
                nn.ConvTranspose2d(self.hidden_channels, self.hidden_channels, **cnn_kwargs), nn.ReLU(),
                nn.BatchNorm2d(self.hidden_channels),
                nn.ConvTranspose2d(self.hidden_channels, self.hidden_channels, **cnn_kwargs), nn.ReLU(),
                nn.BatchNorm2d(self.hidden_channels),
                nn.ConvTranspose2d(self.hidden_channels, self.out_channels, **cnn_kwargs)
